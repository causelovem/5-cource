...
to setup
  ...
  [
    ...
    if random-ph?
    [set ph-f random-float 0.01 + 0.01]
  ]

  if not one-ph?
  [
    set nest patch 0 0
    set food n-of food-source patches with
...
to rotate-ant
  ...
  if [p-type] of patch-here = "food"
  [
    set state "backward"
    set color blue

    ifelse not nest-forward?
    [
      ifelse random-angle?
      [rt random-float 360]
      [rt 180]
  ...
  let p patches in-cone vision angle with
  [distance myself > vision / 2]
