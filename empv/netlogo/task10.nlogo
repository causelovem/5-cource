patches-own [free? p-val d-val dist]
...
to setup-map
  ...
  ask patches with [not free?] [set pcolor black]
  ask free-patches [set pcolor yellow]
  reset-ticks
end
...
to setup-target
  ...
  [
    ask free-patches [set dist distance target-patch]
    let dist-max max [dist] of free-patches
    ask free-patches [set p-val 1 - (dist / dist-max)]
  ]
...
to setup-colony
  ...
  let birth-patch one-of free-patches

  ask birth-patch
  [
    sprout-bacterium colony-size
    [
      set size 0.8
      set shape "circle"
      set val eval-me
...
to-report eval-me
  ifelse not swarm?
  [report [p-val] of patch-here]
  [
    let a mean [h distance myself] of bacterium
...
to cross
  ...
  let o-b one-of other bacterium
  let better? p-val < [p-val] of o-b
...
to dispel
  if random-float 1 > 0.03 [stop]

  let n-p one-of free-patches

  set xcor [pxcor] of n-p
  set ycor [pycor] of n-p
end
