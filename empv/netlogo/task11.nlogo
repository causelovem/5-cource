...
to eval-target
  ...
  [
    ...
    if target-function = "rastring"
    [
      set val x * x + y * y + 10 * (2 - cos(360 * x) - cos(360 * y))
    ]
  ]
end
...
to go
  ...
  [
    ...
    if xcor = p-x and ycor = p-y and xcor != g-x and ycor != g-y
    [
      set vx gamma * vx
      set vy gamma * vy
    ]

    if xcor != g-x and ycor != g-y
    [
...
to-report avg-target
  let s reduce + [val] of particles
  let rep s / count particles
  report rep
end
...
"" ""
PENS
"best" 1.0 0 -13791810 true "" "plot best-target"
"avg" 1.0 0 -3844592 true "" "plot avg-target"
