...
to setup-layout
  ...
  if layout = "random"
  [ask n-of 100 patches [set pcolor grey]]
  if layout = "bar"
  [
    ask patches
    [
      if pycor = 0 and abs(pxcor) < 7
      [set pcolor grey]
    ]
  ]
  if layout = "hole"
  [
    ...
    let h random 10 - 5
    ask patch h 0 [set pcolor white]
  ]
  if layout = "two-holes"
  [
    let yc random 7 + 2
    ask patches
    [
      if pycor = yc or pycor = (- yc)
      [set pcolor grey]
    ]

    let h random 10 - 5
    ask patch h yc [set pcolor white]
  ...
  [
    ...
    ask patches
    [
      if (pycor = yc and pxcor > -5) or (pycor = (- yc) and pxcor < 5)
      [set pcolor grey]
    ]
  ]
...
to eval [w]
  ...
  [
    ...
    ifelse p-a = nobody or [pcolor] of p-a = grey
    [set stop? true]
    [fd 1]
    set k k + 1
  ]
...
to select
  if random-float 1 > psel [stop]
  let o-t one-of other turtles
  let better? f < [f] of o-t
  ifelse better? xor (random-float 1 > pwin)
  [
    ask o-t
    [
      set x [x] of myself
...
to crossover
  if random-float 1 < pcross [stop]
  let o-t one-of other turtles
  let t (list)
  let i 0
  if crossmethod != 1 and crossmethod != 2
  [
    repeat n
    [
      ifelse random-float 1 < pwin
  ...
  [
    let r random (n - 1)
    set t (sublist x r n)
    set x sentence (sublist x 0 r) (sublist [x] of o-t r n)

    ask o-t
    [
      set x sentence (sublist x 0 r) t
    ]
  ]
  if crossmethod = 2
  [
...
;    set x (list)
    ...
    [
      let t2 x
      set x sentence (sublist x 0 r1) (sublist t r1 r2)
      set x sentence x (sublist t2 r2 n)
    ]
  ]
end
...
to mutate
  ...
  [
    ifelse random-float 1 < pwin
    [set t lput (item i x) t]
    [set t lput ((item i x) + (random 4 - 2)) t]
    set i i + 1
  ]
  set x t
end
...
layout
layout
"empty" "random" "bar" "hole" "two-holes" "zigzag"
1
